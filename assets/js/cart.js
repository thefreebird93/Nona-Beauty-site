...
function submitOrder(order){
  try{
    ...
    if(cfg && cfg.emailjs && cfg.emailjs.serviceId && cfg.emailjs.templateId && cfg.emailjs.userId){
      if(window.emailjs){ emailjs.init(cfg.emailjs.userId); emailjs.send(cfg.emailjs.serviceId, cfg.emailjs.templateId, emailData).then(function(){}, function(err){console.error(err);}); }
    }
    // WhatsApp notification
    var wa = (cfg && cfg.socials && cfg.socials.whatsapp) ? cfg.socials.whatsapp : '+201094004720';
    var digits = wa.replace(/\D/g, '');
    if(digits.length <= 10){ digits = '20' + digits; }
    var waLink = 'https://wa.me/' + digits + '?text=' + encodeURIComponent(
      'طلب جديد من الموقع\n' +
      'الاسم: ' + (emailData.customer_name||'') + '\n' +
      'الموبايل: ' + (emailData.customer_phone||'') + '\n' +
      'العنوان: ' + (emailData.customer_address||'') + '\n' +
      'المنتجات: ' + (emailData.items||'') + '\n' +
      'الإجمالي: ' + (emailData.total||'')
    );
